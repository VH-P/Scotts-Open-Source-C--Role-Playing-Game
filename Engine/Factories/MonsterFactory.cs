...
namespace Engine.Factories
{
    public static class MonsterFactory
    {
        ...
        private static void LoadMonstersFromNodes(XmlNodeList nodes,string rootImagePath)
        {
            ...
            {
                Monster monster= new Monster(node.AttributeAsInt("ID"),node.AttributeAsString("Name"),$".{rootImagePath}{node.AttributeAsString("ImageName")}",node.AttributeAsInt("MaximumHitPoints"),ItemFactory.CreateGameItem(node.AttributeAsInt("WeaponID")),node.AttributeAsInt("RewardXP"),node.AttributeAsInt("Gold"));
